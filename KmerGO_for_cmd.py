...
def get_parameters():
    usage_info = "KmerGO [optional options] -i <input_files_folder> -o <output_files_folder>"
    parser = argparse.ArgumentParser(prog='KmerGO', usage=usage_info)
    parser.add_argument('-i', dest='input_path', required=True, type=str, help='input files path')
    parser.add_argument('-o', dest='output_path', required=True, type=str, help='output files path')
    parser.add_argument('-s', dest='step', type=int, default=0, help='step0: all, step1: kmc3, step2: union, step3: filtering, step4: cap3 (default: 0)')
    parser.add_argument('-t', dest='trait_information_path', type=str, default='', help='a csv file path of trait information')
    parser.add_argument('-m', '--mode', dest='mode', type=int, default=0, help='mode: 0-categorical, 1-continuous (default: 0)')
    parser.add_argument('-k', '--kmerlength', dest='k_value', type=int, default=40, help='k-mer length (k from 1 to 256; default: 40)')
...
def KMC_GO(param):
    if param.step == 0:
        try:
            ...
            pass
        _output = 'kmer_countings'
    else:
        _output = param.output_path
    kmc_thread = kmc_read.KMC_Thread((param.k_value, param.ci_value,
                                      param.cs_value, param.input_path,
                                      _output))
    kmc_thread.start()
    last_info = ''
...
def GM_GO(param):
    if param.step == 0:
        try:
            os.mkdir('kmer_matrixes')
        except:
            pass
        _input = 'kmer_countings'
        _output = 'kmer_matrixes'
    else:
        _input = param.input_path
        _output = param.output_path
    try:
        shutil.rmtree('temp')
    except:
    ...
    tiplen = 0
    if param.step != 0:
        gm_thread = kmer_matrix.GM_Thread((_input, _output,
                                           param.process_number, None,
                                           None, None))
    else:
        if param.mode == 0:
            gm_thread = kmer_matrix.GM_Thread((_input, _output,
...
def GF_GO(param):
    if param.step == 0:
        try:
            ...
            pass
        _input = 'kmer_matrixes'
        _output = 'kmer_features'
    else:
        _input = param.input_path
        _output = param.output_path
    try:
    ...
    gf_thread = kmer_features.GF_Thread((_input, _output,
...
def KA_GO(param):
    if param.step == 0:
        _input = 'kmer_features'
    else:
        _input = param.input_path
    ka_thread = sequence_assembly.KA_Thread((_input, param.output_path))
    ka_thread.start()
    print('Step4: k-mer assembly', flush=True)
    while True:
        if ka_thread.status <= 0:
            if ka_thread.status == 0:
                print('Done! Result files are stored in \"' + param.output_path + '\".', flush=True)
                return 0
            else:
...
if __name__ == "__main__":
    ...
    if param.step > 4 or param.step < 0:
        print('Error! Step must be between 0 and 4!', flush=True)
        os._exit(-1)
    if param.mode != 0 and param.mode != 1:
        print('Error! Mode must be 0 or 1!', flush=True)
        os._exit(-1)
    r = 0
    if param.step == 0 or param.step == 3:
        r = Check_csv_validity(param)
    if r == 0 and (param.step == 0 or param.step == 1):
        r = KMC_GO(param)
    if r == 0 and (param.step == 0 or param.step == 2):
        r = GM_GO(param)
    if r == 0 and (param.step == 0 or param.step == 3):
        r = GF_GO(param)
    if r == 0 and (param.step == 0 or param.step == 4):
        KA_GO(param)
